...
@app.route("/api/v1.0/temp_stats_start/<date>/")
#@app.route("/api/v1.0/temp_stats_start/2010-08-23/")
def temp_stats_start(date):
    # Create our session (link) from Python to the DB
    session = Session(engine)

    """Return the min temperature, the avg temperature, and the max temperature for a given start date"""
    # Query temperature observations for all stations    
